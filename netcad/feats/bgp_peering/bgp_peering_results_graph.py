...
class BgpPeeringResultsGrapher(ServiceReporting):
    ...
    def _build_bgp_neighbor_peering(self):
        ...
        hn_to_alias = {dev.name: alias for alias, dev in self.design.devices.items()}
        ...
        for dev, dev_neis_rmap in build.items():
            for if_ipaddr, nei_r in dev_neis_rmap.items():
                ...
                if rmt_nei := build.get(rmt_dev):
                    rmt_nei_r = rmt_nei[rmt_nei_ip]
                    self.add_graph_edge(
                        source=nei_r, target=rmt_nei_r, status=nei_r.status
                    )
